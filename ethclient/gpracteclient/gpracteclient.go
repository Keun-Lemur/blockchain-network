...
// Package practclient provides an RPC client for pract-specific APIs.
package practclient
...
// Client is a wrapper around rpc.Client that implements pract-specific functionality.
//
// If you want to use the standardized practeum RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg practeum.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg practeum.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a pract node.
...
// MemStats retrieves the current memory stats from a pract node.
...
// GetNodeInfo retrieves the node info of a pract node.
