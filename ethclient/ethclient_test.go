...
// Verify that Client implements the practeum interfaces.
var (
	_ = practeum.ChainReader(&Client{})
	_ = practeum.TransactionReader(&Client{})
	_ = practeum.ChainStateReader(&Client{})
	_ = practeum.ChainSyncReader(&Client{})
	_ = practeum.ContractCaller(&Client{})
	_ = practeum.GasEstimator(&Client{})
	_ = practeum.GasPricer(&Client{})
	_ = practeum.LogFilterer(&Client{})
	_ = practeum.PendingStateReader(&Client{})
	// _ = practeum.PendingStateEventer(&Client{})
	_ = practeum.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create practeum Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new practeum service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == practeum.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != practeum.NotFound {
		t.Fatal("error should be practeum.NotFound")
	}
}
