...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of pract is "pract". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Pract", keep that.
	if name == "pract" || name == "pract-testnet" {
		name = "Pract"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "pract" instances.
var isOldPractResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by pract 1.4 are used if they exist.
	if warn, isOld := isOldPractResource[path]; isOld {
		oldpath := ""
		if c.name() == "pract" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldPractResourceWarning, "Using deprecated resource file %s, please move this file to the 'pract' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-practeum-keystore")
		isEphemeral = true
	}
