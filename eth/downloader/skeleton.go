// Copyright 2022 The go-practeum Authors
...
// the process. This is usually the case when Pract is shutting down and some events
...
// data on disk when two subchains are joined togpract.
...
//
// Opposed to the original practeum block synchronization which is trustless (and
...
// not from within (vs. legacy practeum sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Pract is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Pract requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for len(s.progress.Subchains) > 0 {
				// If the last chain is above the new head, delete altogpract
