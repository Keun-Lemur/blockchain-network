...
// information related to the practeum protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(practeum.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as practeum.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria practeum.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(practeum.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
