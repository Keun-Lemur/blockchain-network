// Copyright 2015 The go-practeum Authors
...
import (
	...
	"github.com/practeum-network-network/go-practeum/accounts/abi"
	"github.com/practeum-network-network/go-practeum/accounts/keystore"
	"github.com/practeum-network-network/go-practeum/accounts/scwallet"
	"github.com/practeum-network-network/go-practeum/common"
	"github.com/practeum-network-network/go-practeum/common/hexutil"
	"github.com/practeum-network-network/go-practeum/common/math"
	"github.com/practeum-network-network/go-practeum/consensus/ethash"
	"github.com/practeum-network-network/go-practeum/consensus/misc"
	"github.com/practeum-network-network/go-practeum/core"
	"github.com/practeum-network-network/go-practeum/core/state"
	"github.com/practeum-network-network/go-practeum/core/types"
	"github.com/practeum-network-network/go-practeum/core/vm"
	"github.com/practeum-network-network/go-practeum/crypto"
	"github.com/practeum-network-network/go-practeum/eth/tracers/logger"
	...
	"github.com/practeum-network-network/go-practeum/rlp"
	"github.com/practeum-network-network/go-practeum/rpc"
	"github.com/tyler-smith/go-bip39"
)

// practeumAPI provides an API to access practeum related information.
type practeumAPI struct {
	b Backend
}

// NewpracteumAPI creates a new practeum protocol API.
func NewpracteumAPI(b Backend) *practeumAPI {
	return &practeumAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *practeumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *practeumAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *practeumAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *practeumAPI) Syncing() (interface{}, error) {
...
// practeumAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type practeumAccountAPI struct {
	am *accounts.Manager
}

// NewpracteumAccountAPI creates a new practeumAccountAPI.
func NewpracteumAccountAPI(am *accounts.Manager) *practeumAccountAPI {
	return &practeumAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *practeumAccountAPI) Accounts() []common.Address {
...
// Sign calculates an practeum ECDSA signature for:
// keccak256("\x19practeum Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/practeum-network-network/go-practeum/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19practeum Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/practeum-network-network/go-practeum/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid practeum signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and pract.
...
// BlockChainAPI provides an API to access practeum blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new practeum blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current practeum chain config.
//
...
// See: https://github.com/practeum-network-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/practeum-network-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of practeum APIs exposed over the debugging
// namespace.
...
// Version returns the current practeum protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Pract)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f pract) exceeds the configured cap (%.2f pract)", feeFloat, cap)
	}
	return nil
}
