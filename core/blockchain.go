...
// Package core implements the practeum consensus protocol.
package core
...
import (
	...
	"github.com/practeum-network-network/go-practeum/common/prque"
	...
	"github.com/practeum-network-network/go-practeum/core/state/snapshot"
	...
	"github.com/practeum-network-network/go-practeum/internal/syncx"
	...
	"github.com/practeum-network-network/go-practeum/trie"
...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // Whether to disable trie write caching and GC altogpract (archive node)
...
// available in the database. It initialises the default practeum Validator
// and Processor.
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, root common.Hash, repair bool) (uint64, error) {
	...
	// touching the header chain altogpract, unless the freezer is broken
	if repair {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Pract is initialized with
...
// it's not checked in the Pract side anymore.
...
// sync, Pract will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Pract with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Pract to a new version which supports transaction
