...
import (
	...
	"github.com/practeum-network-network/go-practeum/les/utils"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // practeum protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local practeum node is running in archive mode, advertise ourselves we have
...
// participating in the Light practeum sub-protocol.
