...
// Package les implements the Light practeum Subprotocol.
package les
...
import (
	...
	"github.com/practeum-network-network/go-practeum/common/mclock"
	"github.com/practeum-network-network/go-practeum/consensus"
	"github.com/practeum-network-network/go-practeum/core"
	"github.com/practeum-network-network/go-practeum/core/bloombits"
	...
	"github.com/practeum-network-network/go-practeum/internal/shutdowncheck"
	"github.com/practeum-network-network/go-practeum/les/downloader"
	"github.com/practeum-network-network/go-practeum/les/vflux"
	vfc "github.com/practeum-network-network/go-practeum/les/vflux/client"
	"github.com/practeum-network-network/go-practeum/light"
	...
	"github.com/practeum-network-network/go-practeum/p2p/enr"
...
func New(stack *node.Node, config *ethconfig.Config) (*Lightpracteum, error) {
...
func (s *Lightpracteum) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *Lightpracteum) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *Lightpracteum) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the practeum package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *Lightpracteum) APIs() []rpc.API {
...
func (s *Lightpracteum) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *Lightpracteum) BlockChain() *light.LightChain      { return s.blockchain }
func (s *Lightpracteum) TxPool() *light.TxPool              { return s.txPool }
func (s *Lightpracteum) Engine() consensus.Engine           { return s.engine }
func (s *Lightpracteum) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *Lightpracteum) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *Lightpracteum) EventMux() *event.TypeMux           { return s.eventMux }
func (s *Lightpracteum) Merger() *consensus.Merger          { return s.merger }

// Protocols returns all the currently configured network protocols to start.
func (s *Lightpracteum) Protocols() []p2p.Protocol {
...
// light practeum protocol implementation.
func (s *Lightpracteum) Start() error {
...
// practeum protocol.
func (s *Lightpracteum) Stop() error {
	close(s.closeCh)
	s.serverPool.Stop()
	s.peers.close()
	s.reqDist.close()
	...
	s.lesDb.Close()
	s.wg.Wait()
	log.Info("Light practeum stopped")
	return nil
}
