...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Pract.LightScryptN, Pract.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.practeum", new NodeConfig());
			node.start();
			...
			practeumClient ec = node.getpracteumClient();
	...
	// signatures too. Regression test for go-practeum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogpract
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/practeum/practtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Pract and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.practeum", "github.com/practeum-network-network/go-practeum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "pract.aar"), "pract.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "practeum", "practtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.practeum.practtest"
