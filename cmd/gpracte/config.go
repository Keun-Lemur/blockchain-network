...
import (
	...
	"github.com/practeum-network-network/go-practeum/accounts/external"
	...
	"github.com/practeum-network-network/go-practeum/accounts/usbwallet"
...
func loadConfig(file string, cfg *practConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "pract.ipc"
	return cfg
}

// makeConfigNode loads pract configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, practConfig) {
	// Load defaults.
	cfg := practConfig{
...
// makeFullNode loads pract configuration and creates the practeum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `pract db freezer-migrate`.")
		}
	}
	...
	// Add the practeum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *practConfig) {
