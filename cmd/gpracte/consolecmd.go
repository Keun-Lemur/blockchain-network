...
import (
	...
	"github.com/practeum-network-network/go-practeum/console"
...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The Pract console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://pract.ethereum.org  /docs/interface/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://pract.ethereum.org  /docs/interface/javascript-console.
This command allows to open a console on a running pract node.`,
	}
...
JavaScript API. See https://pract.ethereum.org  /docs/interface/javascript-console`,
	}
)

// localConsole starts a new pract node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to attach to the inproc pract: %v", err)
	}
...
// remoteConsole will connect to a remote pract instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote pract: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new pract node, attaches an ephemeral JavaScript
...
pract --exec "%s" console`, b.String())
	return nil
}

// dialRPC returns a RPC client which connects to the given endpoint.
// The check for empty endpoint implements the defaulting logic
// for "pract attach" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with pract < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
