...
`
	...
	{
		pract := runPract(t, "account", "list", "--datadir", datadir)
		pract.Expect(want)
		pract.ExpectExit()
	}
	{
		pract := runPract(t, "--datadir", datadir, "account", "list")
		pract.Expect(want)
		pract.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	pract := runPract(t, "account", "new", "--lightkdf")
	defer pract.ExpectExit()
	pract.Expect(`
...
func TestAccountHelp(t *testing.T) {
	pract := runPract(t, "account", "-h")
	pract.WaitExit()
	if have, want := pract.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	pract = runPract(t, "account", "import", "-h")
	pract.WaitExit()
	if have, want := pract.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	pract := runPract(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer pract.ExpectExit()
	pract.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	pract := runPract(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	pract := runPract(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer pract.ExpectExit()
	pract.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(pract.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	pract := runMinimalPract(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(pract.StderrText(), m) {
...
// https://github.com/practeum-network-network/go-practeum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	pract.SetTemplateFunc("keypath", func(file string) string {
