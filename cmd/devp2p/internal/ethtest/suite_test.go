...
import (
	...
	"github.com/practeum-network-network/go-practeum/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	pract, err := runPract()
	if err != nil {
		t.Fatalf("could not run pract: %v", err)
	}
	defer pract.Close()

	suite, err := NewSuite(pract.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runPract creates and starts a pract node
func runPract() (*node.Node, error) {
...
func setupPract(stack *node.Node) error {
