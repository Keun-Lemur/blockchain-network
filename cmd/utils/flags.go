...
// Package utils contains internal helper functions for go-practeum commands.
package utils
...
import (
	...
	"github.com/practeum-network-network/go-practeum/common/fdlimit"
	"github.com/practeum-network-network/go-practeum/consensus"
	"github.com/practeum-network-network/go-practeum/consensus/ethash"
	"github.com/practeum-network-network/go-practeum/core"
	"github.com/practeum-network-network/go-practeum/core/rawdb"
	"github.com/practeum-network-network/go-practeum/core/vm"
	"github.com/practeum-network-network/go-practeum/crypto"
	"github.com/practeum-network-network/go-practeum/eth"
	ethcatalyst "github.com/practeum-network-network/go-practeum/eth/catalyst"
	...
	"github.com/practeum-network-network/go-practeum/eth/filters"
	"github.com/practeum-network-network/go-practeum/eth/gasprice"
	"github.com/practeum-network-network/go-practeum/eth/tracers"
	"github.com/practeum-network-network/go-practeum/ethdb"
	"github.com/practeum-network-network/go-practeum/ethdb/remotedb"
	"github.com/practeum-network-network/go-practeum/ethstats"
	"github.com/practeum-network-network/go-practeum/graphql"
	"github.com/practeum-network-network/go-practeum/internal/ethapi"
	"github.com/practeum-network-network/go-practeum/internal/flags"
	"github.com/practeum-network-network/go-practeum/les"
	lescatalyst "github.com/practeum-network-network/go-practeum/les/catalyst"
	"github.com/practeum-network-network/go-practeum/log"
	"github.com/practeum-network-network/go-practeum/metrics"
	"github.com/practeum-network-network/go-practeum/metrics/exp"
	"github.com/practeum-network-network/go-practeum/metrics/influxdb"
	"github.com/practeum-network-network/go-practeum/miner"
	"github.com/practeum-network-network/go-practeum/node"
	"github.com/practeum-network-network/go-practeum/p2p"
	"github.com/practeum-network-network/go-practeum/p2p/enode"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in pract) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Pract will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Pract configurations storing the
...
// for Pract and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `pract account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Practeum) {
	if cfg.SyncMode == downloader.LightSync {
		...
		if err != nil {
			Fatalf("Failed to register the practeum service: %v", err)
		}
...
// RegisterEthStatsService configures the practeum Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the practeum Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "pract.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "pract.", tagsMap)
		}
